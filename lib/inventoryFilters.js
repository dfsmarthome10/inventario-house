export const MAIN_CATEGORIES = ["cajas", "herramientas", "comida", "casa", "gabinete", "otros"];
...
export const GABINETE_SUBCATEGORIES = ["gavetero_principal", "gavetero_1", "gavetero_2", "gavetero_3", "gavetero_4"];
...
export function buildInventorySummary(items) {
  ...
  const houseByZone = HOUSE_SUBCATEGORIES.reduce((acc, zone) => {
    acc[zone] = items.filter((item) => item.categoria_principal === "casa" && item.subcategoria === zone).length;
    return acc;
  }, {});
  const gabineteByZone = GABINETE_SUBCATEGORIES.reduce((acc, zone) => {
    acc[zone] = items.filter((item) => item.categoria_principal === "gabinete" && item.subcategoria === zone).length;
    return acc;
  }, {});
...
export function getCategoryOptionsFromItems(items) {
  ...
  const subSet = new Set([...FOOD_SUBCATEGORIES, ...HOUSE_SUBCATEGORIES, ...GABINETE_SUBCATEGORIES]);
