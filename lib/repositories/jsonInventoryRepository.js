...
const MAIN_CATEGORIES = ["cajas", "herramientas", "comida", "otros"];
...
function normalizeItem(raw) {
  ...
  const subcategoria = categoria_principal === "comida" ? normalizeTag(derived.subcategoria, "") || null : null;
...
function compareItems(a, b) {
  ...
  const subOrderA = FOOD_SUBCATEGORIES.includes(a.subcategoria || "") ? FOOD_SUBCATEGORIES.indexOf(a.subcategoria) : FOOD_SUBCATEGORIES.length;
  const subOrderB = FOOD_SUBCATEGORIES.includes(b.subcategoria || "") ? FOOD_SUBCATEGORIES.indexOf(b.subcategoria) : FOOD_SUBCATEGORIES.length;
...
function groupItemsByCategory(items) {
  ...
  MAIN_CATEGORIES.forEach((cat) => {
    grouped[cat] = { categoria_principal: cat, items: [], subcategorias: cat === "comida" ? { lacena: [], nevera: [], congelador: [] } : {} };
  });

  items.forEach((item) => {
    const mainCategory = item.categoria_principal || "otros";
    if (!grouped[mainCategory]) {
      grouped[mainCategory] = { categoria_principal: mainCategory, items: [], subcategorias: mainCategory === "comida" ? { lacena: [], nevera: [], congelador: [] } : {} };
    }

    if (mainCategory === "comida") {
      if (item.subcategoria && grouped.comida.subcategorias[item.subcategoria]) {
        grouped.comida.subcategorias[item.subcategoria].push(item);
      } else {
        grouped.comida.items.push(item);
      }
