...
export async function getOrCreateOpenSession(scope) {
  const { scope: normalizedScope } = getScopeConfig(scope);
...
export async function getCatalogByScope(scope, filters = {}) {
  ...
  return filterItems(data || [], filters, subcategories);
}
...
export async function upsertCartLine({ sessionId, inventoryItemId, quantityToBuy, purchasePrice, taxApplies }) {
  ...
  if ((item.categoria_principal || "").toString().toLowerCase() !== session.scope) {
    throw new Error("Inventory item scope mismatch for this shopping session.");
  }
...
export async function confirmPurchase(sessionId, scope) {
  const { scope: normalizedScope } = getScopeConfig(scope);
  const session = await getSessionScope(sessionId);

  if (session.scope !== normalizedScope) {
    throw new Error(`Session scope mismatch. Expected ${normalizedScope}, received ${session.scope}.`);
  }
  ...
  for (const line of lines) {
    ...
    if ((line.item.categoria_principal || "").toString().toLowerCase() !== normalizedScope) {
      throw new Error(`Item ${line.inventory_item_id} no pertenece a ${normalizedScope}.`);
    }
...
export async function getPurchaseHistory(filters = {}) {
  ...
  const { data: linesRaw, error: linesError } = await supabase
    .from(RECEIPT_ITEMS_TABLE)
    .select("receipt_id, item_nombre, subcategoria, quantity_purchased")
...
export async function getPurchaseHistoryCalendar(month, filters = {}) {
  ...
  const { data: linesRaw, error: linesError } = await supabase
    .from(RECEIPT_ITEMS_TABLE)
    .select("receipt_id, subcategoria, quantity_purchased, line_total")
    .in("receipt_id", receiptIds);
  ...
  mappedReceipts.forEach((receipt) => {
...
export async function getFoodPurchaseSignals(daysBack = 120) {
  ...
  const { data: receiptsRaw, error: receiptsError } = await supabase
    .from(RECEIPTS_TABLE)
    .select("id, created_at, session:purchase_sessions(scope)")
  ...
  const receipts = (receiptsRaw || []).filter((receipt) => receipt.session?.scope === "comida");
  if (receipts.length === 0) {
    return {
      days_back: safeDays,
      total_receipts: 0,
  ...
  const { data: linesRaw, error: linesError } = await supabase
    .from(RECEIPT_ITEMS_TABLE)
    .select("receipt_id, inventory_item_id, item_nombre, subcategoria, quantity_purchased")
    .in("receipt_id", receiptIds);
  ...
  for (const line of linesRaw || []) {
    if (!["lacena", "nevera", "congelador"].includes((line.subcategoria || "").toString().toLowerCase())) {
      continue;
    }
...
export async function getOpenCartBadgeCounts() {
  ...
  const { data: sessions, error: sessionsError } = await supabase
    ...
    .select("id, scope")
    .eq("status", "open")
    .in("scope", ["comida", "casa"]);

  if (sessionsError) {
    throw new Error(`Failed to fetch open cart sessions: ${sessionsError.message}`);
  }

  const scopeBySessionId = new Map();
  (sessions || []).forEach((session) => {
    scopeBySessionId.set(session.id, session.scope);
  });

  const sessionIds = Array.from(scopeBySessionId.keys());
  ...
  let casaLines = 0;
  (lines || []).forEach((line) => {
    const scope = scopeBySessionId.get(line.session_id);
    if (scope === "comida") {
      comidaLines += 1;
    } else if (scope === "casa") {
      casaLines += 1;
...
export async function getOrCreateOpenFoodSession() {
  return getOrCreateOpenSession("comida");
}

export async function getOrCreateOpenHouseSession() {
  return getOrCreateOpenSession("casa");
}
...
export async function confirmFoodPurchase(sessionId) {
  return confirmPurchase(sessionId, "comida");
}

export async function confirmHousePurchase(sessionId) {
  return confirmPurchase(sessionId, "casa");
}
