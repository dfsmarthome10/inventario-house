...
function filterFoodItems(items, rawFilters) {
...
export async function getFoodCatalog(filters = {}) {
  ...
  const { data, error } = await supabase
    ...
    .eq("categoria_principal", FOOD_SCOPE)
    .order("subcategoria", { ascending: true })
    .order("nombre", { ascending: true });

  if (error) {
    throw new Error(`Failed to fetch food catalog: ${error.message}`);
  }

  const catalog = (data || []).filter((item) => FOOD_ZONES.includes(item.subcategoria || ""));
  return filterFoodItems(catalog, filters);
}
...
export async function getSessionCart(sessionId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, session_id, inventory_item_id, quantity_to_buy, purchase_price, tax_applies, tax_rate, line_subtotal, line_tax, line_total, created_at, updated_at, inventory_item:inventory_items(id, alias, nombre, subcategoria, cantidad_actual, unidad, thumbnail_url)"
    )
...
export async function confirmFoodPurchase(sessionId) {
  ...
  const { data: linesRaw, error: linesError } = await supabase
    .from(SESSION_ITEMS_TABLE)
    .select("id, inventory_item_id, quantity_to_buy, purchase_price, tax_applies, tax_rate, line_subtotal, line_tax, line_total, inventory_item:inventory_items(id, alias, nombre, subcategoria, cantidad_actual)")
    .eq("session_id", sessionId)
    .order("created_at", { ascending: true });
...
export async function getReceiptById(receiptId) {
  const supabase = getClient();
  const { data: receipt, error: receiptError } = await supabase.from(RECEIPTS_TABLE).select("*").eq("id", receiptId).maybeSingle();
...
export async function getPurchaseHistory(filters = {}) {
  ...
  const { data: receiptsRaw, error: receiptsError } = await supabase
    .from(RECEIPTS_TABLE)
    .select("id, created_at, subtotal_amount, tax_total, grand_total, total_amount, line_count")
  ...
  return receipts
    .map((receipt) => {
      ...
      const searchable = `${receipt.id} ${itemNames.join(" ")}`.toLowerCase();
...
export async function getFoodPurchaseSignals(daysBack = 120) {
  ...
  for (const line of linesRaw || []) {
    if (!FOOD_ZONES.includes((line.subcategoria || "").toString().toLowerCase())) {
      continue;
    }
